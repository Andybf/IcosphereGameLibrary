...
void Shader::setUniformTexture(uint textureId, uint dimensions) {
    AP_TEST(glUniform1i(glGetUniformLocation(activeShaderId, "texture0"),textureId-1));
...
void setUniformMatrix(cchar* name, glm::mat4 matrix) {
    AP_TEST(glUniformMatrix4fv(glGetUniformLocation(activeShaderId,name), AP_COUNT, GL_FALSE, &matrix[0][0]));
}
void Shader::setUniformIntArray(cchar* name, int* intArray, int size) {
    AP_TEST(glUniform1iv(glGetUniformLocation(activeShaderId,name), size*sizeof(int), intArray));
}
...
void Shader::setUniformVec3(cchar* name, glm::vec3 value) {
    AP_TEST(glUniform3f(glGetUniformLocation(activeShaderId,name),value.x,value.y,value.z));
}
